...
int send_rcon(SOCKET sock, int id, int command, char *string1, char *string2) {
	int size;
	int ret;
	...
	ret = send(sock, (char*)&id, sizeof(int), 0);
	if (ret == -1) {
		perror("send() failed:");
		return -1;
	}
	ret = send(sock, (char*)&command, sizeof(int), 0);
	if (ret == -1) {
		perror("send() failed:");
		return -1;
	}
...
int recv_rcon(SOCKET sock, int timeout, int *id, int *command, char *string1, char *string2) {
	struct timeval tv;
	fd_set readfds;
	int size;
	char *ptr;
	...
	*command = 0xDEADBEEF;
	...
	select(((int)sock) + 1, &readfds, NULL, NULL, &tv);
...
int process_response(SOCKET sock) {
	int ret;
	int id;
	int command;
