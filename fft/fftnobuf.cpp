...
void cycleLeader(vector<T> &v, const size_t start, const size_t len)
{
    for (size_t i = 1, j = 1; i < len; i *= 3, j = i)
        do {
            j = (j & 1) ? (j + len)/2 : j/2;
            std::swap(v[start + j], v[start + i]);
        } while (j != i);
}

template<typename T>
void unshuffle(vector<T> &v, const size_t start, const size_t length)
{
    for (size_t s = 0, len; s < length; s += len)
    {
        len = getBiggest3k1(length - s);
        cycleLeader (v, start + s, len);
        rotate(v.begin() + start / 2, v.begin() + start, v.begin() + start + (length - length / 2));
    }
}
    ...
    const bool inverse)
{
    ...
    Complex wk = 1;
    unshuffle(P, start, length);

    // Recursion on half the arrays, giving that sweet O(nlog(n)) complexity
    _FFT(P, res, start, length/2, inverse);
    _FFT(P, res, start + length/2, length/2, inverse);
