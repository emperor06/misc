...
function FFT(P, inverse) {
    const n = P.length
    if (n == 1) {
        return typeof P[0] === "object" ? P : [complex(P[0], 0)]
    }
    const phi = inverse ? -tau/n : tau/n
    const sin = Math.sin(phi)
    const cos = Math.sqrt(1 - sin*sin)
    const w = complex(cos, sin)
    ...
    let wk = complex(1, 0)
    for (let j = 0; j < (n>>1); j++) {
        y[j] = add(ye[j], multiply(wk, yo[j]))
        y[j + (n>>1)] = add(ye[j], multiply(wk, yo[j]).neg())
        wk.mul(w)
    }
    return y
}
