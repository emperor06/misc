...
function generatePlayerScav() {
    ...
    let securedContainer = items.find(item => item.slotId === "SecuredContainer")

    if (securedContainer) {
        let toRemove = itm_hf.getChildren(items, securedContainer._id);
        scavData[0].Inventory.items = items.filter(item => !toRemove.includes(item._id));
    }
